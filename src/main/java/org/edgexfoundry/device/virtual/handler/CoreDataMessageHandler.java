...
public class CoreDataMessageHandler {
			...
			if (event != null) {
				try {
					...
					logger.debug("Problem sending event for " + event.getDevice()
							+ " to core data.  Retrying (attempt " + (attempt + 1) + ")..." + event.toString(), e);
					try {
						Thread.sleep(delay);
	...
	public List<ResponseObject> sendCoreData(String deviceName, List<Reading> readings, Map<String, VirtualObject> objects) {
		try{
