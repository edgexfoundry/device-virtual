...
import org.edgexfoundry.device.virtual.domain.SimpleSchedule;
import org.edgexfoundry.device.virtual.domain.SimpleScheduleEvent;
...
public class Scheduler
{
	...
	{
		// do not schedule any new event when the application is shutting down
			...
			while (scheduleContextQueue.peek() != null && scheduleContextQueue.peek().getNextTime().toEpochSecond() <= nowEpoch)
			{
				try {
					...
					if(scheduleContext.isComplete()) {
						logger.debug("schedule " + scheduleContext.getInfo() + " is complete." + scheduleContext.toString());
					} else {
